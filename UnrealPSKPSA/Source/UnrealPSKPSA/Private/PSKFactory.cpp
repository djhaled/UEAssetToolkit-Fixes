...
UObject* UPSKFactory::Import(const FString Filename, UObject* Parent, const FName Name, const EObjectFlags Flags, USkeleton* Skeleton) const
{
	...
	for (const auto PskFace : Psk.Faces)
	{
		...
		for (auto VertexIndex : WindingOrder)
		{
			...
			Wedge.UVs[0] = FVector2D(PskWedge.U, PskWedge.V);
			...
			Face.TangentZ[VertexIndex] = Psk.bHasVertexNormals ? Psk.Normals[PskWedge.PointIndex] : FVector::ZeroVector;
			Face.TangentY[VertexIndex] = FVector::ZeroVector;
			Face.TangentX[VertexIndex] = FVector::ZeroVector;
		}
	...
	for (auto PskBone : Psk.Bones)
	{
		...
		PskTransform.SetLocation(FVector(PskBonePos.Position.X, -PskBonePos.Position.Y, PskBonePos.Position.Z));
		PskTransform.SetRotation(FQuat(PskBonePos.Orientation.X, -PskBonePos.Orientation.Y, PskBonePos.Orientation.Z,
			(PskBone.ParentIndex == -1) ? -PskBonePos.Orientation.W : PskBonePos.Orientation.W ).GetNormalized());
	...
	SkeletalMesh->SetImportedBounds(FBoxSphereBounds(FBoxSphereBounds(FBox(SkeletalMeshImportData.Points))));
	...
	// TODO: Create physics asset (can just be a temporary one for now)
